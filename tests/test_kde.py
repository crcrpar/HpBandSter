...
class BaseNdTest(object):
    ...
    def setUp(self):
        ...
        self.sm_1d_kdes = [
            sm.nonparametric.KDEMultivariate(data=self.sm_x_train[:, i],
                                             var_type=self.var_types[i],
                                             bw='cv_ml') for i in range(len(self.var_types))
        ]
    ...
    def test_bandwidths_estimation(self):
        ...
        for d in range(len(self.var_types)):
            ...
            self.assertAlmostEqual(self.sm_1d_kdes[d].bw[0],
                                   self.hp_kde_factor.bandwidths[d],
                                   delta=5e-2)
    ...
    def test_loo_likelihood(self):
        for bw in np.logspace(-1, -0.1, 5):
            ...
            sm_full_ll = self.sm_kde.loo_likelihood(bw=np.array([bw] * len(self.var_types)),
                                                    func=np.log)
            hp_full_ll = self.hp_kde_full.loo_negloglikelihood()
            hp_factor_ll = self.hp_kde_factor.loo_negloglikelihood()
            ...
            self.assertAlmostEqual(np.sum((sm_factor_ll / n) + np.log(n - 1)),
                                   hp_factor_ll,
                                   delta=delta)
