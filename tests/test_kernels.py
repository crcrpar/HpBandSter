...
class Test1dCategorical(unittest.TestCase):
    ...
    def test_values(self):
        for bw in [1e-3, 1e-2, 1e-1, 1]:
            ...
            hp_kernel = hp_kernels.AitchisonAitken(
                data=self.x_train,
                bandwidth=bw,
...
class Test1dInteger(unittest.TestCase):
    ...
    @unittest.skipIf(rapid_development, "test skipped to accelerate developing new tests")
    def test_values(self):
        n = self.configspace.get_hyperparameters()[0].upper - self.configspace.get_hyperparameters(
        )[0].lower + 1
        ...
        for bw in [1e-3, 1e-2, 1e-1, 0.99]:
            ...
            sm_values = np.array(
                [sm_kernels.wang_ryzin(bw, sm_x_train[:, None], x) for x in sm_x_test]).squeeze()
            hp_kernel = hp_kernels.WangRyzinInteger(data=self.x_train,
                                                    bandwidth=bw,
                                                    num_values=n,
                                                    fix_boundary=False)
...
class Test1dOrdinal(unittest.TestCase):
    ...
    def test_values(self):
        ...
        for bw in [1e-3, 1e-2, 1e-1, 1]:
            sm_values = np.array(
                [sm_kernels.wang_ryzin(bw, self.x_train[:, None], x) for x in self.x_test])
            hp_kernel = hp_kernels.WangRyzinOrdinal(data=self.x_train,
                                                    bandwidth=bw,
                                                    fix_boundary=False)
            hp_values = hp_kernel(self.x_test)
            self.assertTrue(np.allclose(hp_values.T, sm_values, 1e-4))
