...
def performance_histogram_model_vs_random(runs, id2conf, show=False):
    model_based_runs = list(
        filter(lambda r: id2conf[r.config_id]['config_info']['model_based_pick'], runs))
    random_runs = list(
        filter(lambda r: not id2conf[r.config_id]['config_info']['model_based_pick'], runs))
    ...
    for i, b in enumerate(budgets):
        mbax, rax = axarr[i]
        mbax.hist(losses[b]['model_based'],
...
def interactive_HBS_plot(learning_curves,
                         tool_tip_strings=None,
                         log_y=False,
                         log_x=False,
                         reset_times=False,
                         color_map='Set3',
    ...
    hover_annotation = ax.annotate("BLABLA",
                                   xy=(0, 0),
                                   xytext=(20, 20),
    ...
    def onpick1(event):
        ...
        if gid in permanent_annotations:
            ...
            permanent_annotations[gid] = ax.annotate(text,
                                                     copy.deepcopy((xdata[-1], ydata[-1])),
                                                     xytext=(20, 20),
                                                     textcoords="offset points",
