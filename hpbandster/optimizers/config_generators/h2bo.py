...
class H2BO(base_config_generator):
    ...
    def get_config(self, budget):
        ...
        if sample is None:
            try:
                ...
                ei = self.kde_models[budget]['good'].pdf(
                    samples) / self.kde_models[budget]['bad'].pdf(samples)
    ...
    def new_result(self, job, update_model=True):
        ...
        conf = ConfigSpace.Configuration(self.configspace,
                                         job.kwargs["config"]).get_array().tolist()
        ...
        if self.bw_estimator in ['mlcv'] and n_good < 3:
            self.kde_models[budget]['good'].bandwidths[:] = self.kde_models[budget][
                'bad'].bandwidths
