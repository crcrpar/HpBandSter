...
class Gaussian(BaseKernel):
    def _compute_weights(self):
        ...
        for i, d in enumerate(self.data):
            weights[i] = 2. / (erf(
                (1 - d) / (np.sqrt(2) * self.bw)) + erf(d / (np.sqrt(2) * self.bw)))
    ...
    def sample(self, sample_indices=None, num_samples=1):
        ...
        while len(oob_idx) > 0:
            ...
            oob_idx = oob_idx[np.argwhere(np.logical_or(samples[oob_idx] > 1,
                                                        samples[oob_idx] < 0))].flatten()
...
class WangRyzinOrdinal(BaseKernel):
    ...
    def sample(self, sample_indices=None, num_samples=1):
        ...
        while len(oob_idx) > 0:
            ...
            oob_idx = oob_idx[np.argwhere(
                np.logical_or(samples[oob_idx] > self.num_values - 0.9,
                              samples[oob_idx] < -0.1)).flatten()]
...
class WangRyzinInteger(BaseKernel):
    def _compute_weights(self):
        ...
        x_test = np.linspace(1 / (2 * self.num_values),
                             1 - (1 / (2 * self.num_values)),
                             self.num_values,
                             endpoint=True)
        self.weights = 1.
        pdfs = self.__call__(x_test)
    ...
    def __call__(self, x_test):
        ...
        idx = np.abs(distances) < 1 / (3 * self.num_values
                                       )  # distances smaller than that are considered zero
        pdfs[idx] = (1 - self.bw)
        pdfs[~idx] = 0.5 * (1 - self.bw) * np.power(self.bw,
                                                    np.abs(distances[~idx]) * self.num_values)
        # reweigh to compensate for boundaries
        pdfs *= self.weights
    ...
    def sample(self, sample_indices=None, num_samples=1):
        ...
        while len(oob_idx) > 0:
            ...
            samples[oob_idx] += delta[oob_idx]
            oob_idx = oob_idx[np.argwhere(
                np.logical_or(samples[oob_idx] > 1 - 1 / (3 * self.num_values),
