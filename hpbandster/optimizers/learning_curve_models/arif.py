...
class ARIF(lcm_base):
    ...
    def invert_differencing(self, initial_part, differenced_rest, order=None):
        ...
        starting_points = [
            self.apply_differencing(initial_part, order=order)[-1]
...
if __name__ == "__main__":
    ...
    data = load_configs(
        "/home/sfalkner/repositories/bitbucket/learning_curve_prediction/data/fc_net_mnist", 1024)
    ...
    reconstruction = lc_model.invert_differencing(random_sequence[:1 + test_order],
                                                  tmp,
                                                  order=test_order)
    ...
    for i in range(16):
        ...
        pred = lc_model.extend_partial(learning_curves[i],
                                       T_max - len(learning_curves[i]),
                                       config=data[0][i])
        plt.plot(full_lcs[i])
