    ...
    def discover_workers(self):
        ...
        while True:
            ...
            with Pyro4.locateNS(host=self.nameserver, port=self.nameserver_port) as ns:
                ...
                self.logger.debug(
                    "DISPATCHER: Found %i potential workers, %i currently in the pool." %
            ...
            for crashed_job in crashed_jobs:
                self.discover_cond.release()
                self.register_result(crashed_job, {
                    'result': None,
    ...
    def job_runner(self):
        ...
        while True:
            ...
            while self.waiting_jobs.empty() or len(self.idle_workers) == 0:
                self.logger.debug(
                    'DISPATCHER: jobs to submit = %i, number of idle workers = %i -> waiting!' %
