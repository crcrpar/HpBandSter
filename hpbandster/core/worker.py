...
class Worker(object):
    ...
    def __init__(self,
        ...
        self.worker_id = "hpbandster.run_%s.worker.%s.%i" % (self.run_id, socket.gethostname(),
                                                             os.getpid())
    ...
    def load_nameserver_credentials(self, working_directory, num_tries=60, interval=1):
        ...
        for i in range(num_tries):
            try:
                ...
                self.logger.warning('config file %s not found (trail %i/%i)' %
                                    (fn, i + 1, num_tries))
                time.sleep(interval)
            except:
                raise
    ...
    def run(self, background=False):
        ...
        if background:
            ...
            self.thread = threading.Thread(target=self._run,
                                           name='worker %s thread' % self.worker_id)
