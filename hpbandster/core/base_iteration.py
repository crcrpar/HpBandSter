...
class BaseIteration(object):
    ...
    def add_configuration(self, config=None, config_info={}):
        ...
        self.data[config_id] = Datum(config=config,
    ...
    def register_result(self, job, skip_sanity_checks=False):
        ...
        if not skip_sanity_checks:
            ...
            assert d.budget == budget, 'Budgets differ (%f != %f)!' % (
                self.data[config_id]['budget'], budget)
        ...
        for k, v in self.data.items():
            if v.status == 'QUEUED':
                assert v.budget == self.budgets[
                    self.stage], 'Configuration budget does not align with current stage!'
                v.status = 'RUNNING'
                self.num_running += 1
    ...
    def _advance_to_next_stage(self, config_ids, losses):
		...
		"""
        raise NotImplementedError('_advance_to_next_stage not implemented for %s' %
                                  type(self).__name__)
    ...
    def process_results(self):
        ...
        for i, a in enumerate(advance):
            if a:
                self.logger.debug('ITERATION: Advancing config %s to next budget %f' %
    ...
    def finish_up(self):
        ...
        for k, v in self.data.items():
            assert v.status in ['TERMINATED', 'REVIEW',
                                'CRASHED'], 'Configuration has not finshed yet!'
            v.status = 'COMPLETED'
...
class WarmStartIteration(BaseIteration):
    ...
    def __init__(self, Result, config_generator):
        ...
        for i, id in enumerate(id2conf):
            new_id = self.add_configuration(config=id2conf[id]['config'],
